...
import boom.game.GUI;
public class Actor {            //check co the di chuyn dc k va vi tri
	...
	public void drawActor(Graphics2D g2d){              //cac doi tuong thany doi
		switch (type) {
		case BOMBER:
	...
	//xet dieu kien xem cco dc phep di hay khong??
	public boolean move(int count, ArrayList<Bomb> arrBomb, ArrayList<Box> arrBox){
		if(count%speed!=0){
			return true;
		}
		switch (orient) {//neu dich dc 1 khoang la 1 thi co the dich dc con k la k cho dich
		case LEFT:
			if(x<=0){
				return false;
			}
